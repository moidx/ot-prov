...
usage () {
  ...
  echo "  -s <SoftHSMConfig> Path to the SoftHSM config file."
...
certgen () {
  ...
  SOFTHSM2_CONF="${FLAGS_SOFTHSM_CONFIG}" \
...
echo "hsmtool-pin: ${FLAGS_HSMTOOL_PIN}"
...
if [[ "${FLAGS_WIPE}" == true ]]; then
  ...
  "${HSMTOOL_BIN}"                         \
    --logging=info                         \
    --token="${FLAGS_HSMTOOL_TOKEN}"       \
    exec "hsm_spm_destroy.hjson"
fi
